...
class PrometheusWorker extends Base {
  ...
  get config() {
    return this.app.config.exporter;
  }
  ...
  async _createConnection() {
    ...
    conn.on('request', req => {
      ...
      if (req.connObj.type === 'request' && req.connObj.method === 'clearDefaultCollect') {
        // 清除 collectDefaultMetrics
        let isClear = false;
        if (this.app.prometheus && this.app.prometheus.collectInterval) {
          clearInterval(this.app.prometheus.collectInterval);
          this.app.prometheus.collectInterval = null;
          isClear = true;
        }
